...
class DB:
    def __init__(self, host, uuid, dbname, password):
        ...
        if not bool(self.cur.rowcount):
            print("create table")
            self.cur.execute("CREATE TABLE movies (id serial PRIMARY KEY, title VARCHAR, releaseDate VARCHAR, link VARCHAR, runtime VARCHAR, type VARCHAR, poster VARCHAR, trailer VARCHAR, actors VARCHAR, characters VARCHAR, storyline Text);")
