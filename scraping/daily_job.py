...
def getlink(s, date):
	...
	#Get IMDB Path
    # https://www.imdb.com/search/title/?title_type=feature&release_date=2022-06-05&count=250
    ...
    for mv in formalist:
        ...
        mvlist.append({'header': header, 'releaseDate': str(date), 'link': link, 'runtime': runtime, 'type': mvtype})
...
def getDetailJob(s, mv):
    ...
    # mvinfo['releaseDate'] = str(mv.date)
    # ...

    #print(mvinfo['characters'])
    imdb.cur.execute("INSERT INTO movies (title, releaseDate, link, runtime, type, poster, trailer, actors, characters, storyline) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", 
    (mvinfo['title'], mv['releaseDate'], mv['link'], mv['runtime'], mv['type'], mvinfo['poster'], mvinfo['trailer'], mvinfo['actors'], mvinfo['characters'], mvinfo['storyline']))
    # print(mvinfo.keys())
...
for movie in movieList:
    imdb.cur.execute("select count(*) from movies where link=%s", (movie['link'],))
    result = imdb.cur.fetchone()
    if result[0] == 0:
        detailThreads.append(threading.Thread(target = getDetailJob, args = (s, movie)))
        detailThreads[-1].start()
        time.sleep(1)
    else:
        print('Skip ' + movie['header'])
